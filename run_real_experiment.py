...
print("="*80)
print("SCI EXPERIMENT FOR YOUR DATASET STRUCTURE")
print("="*80)
...
train_mask_dir = data_path / "train/masks/images"  # Note: masks are in images subfolder
...
results = []
num_samples = min(50, len(train_images))
...
for i, img_path in enumerate(tqdm(train_images[:num_samples], desc="Processing")):
    ...
    for strategy in strategies:
        ...
        try:
            ...
            print(f"Error processing {strategy} for {img_path.name}: {e}")
            continue
...
EXPERIMENT SETUP:
-----------------
Prompt strategies tested: {', '.join(strategies)}
Images: {len(train_images)} available, {num_samples} tested
Masks: {len(train_masks)} real masks found
Synthetic masks used when real masks unavailable
...
KEY FINDINGS:
-------------
1. Best performing strategy: {best_strategy.upper()}
2. IoU range: {df['iou'].min():.4f} to {df['iou'].max():.4f}
3. Confidence range: {df['confidence'].min():.4f} to {df['confidence'].max():.4f}
4. Correlation between IoU and confidence: {df['iou'].corr(df['confidence']):.4f}

RECOMMENDATIONS:
----------------
1. Use {best_strategy} prompts for maximum accuracy in remote sensing applications
2. Consider confidence scores as reliability indicators
3. Validate performance with ground truth when available
4. Adjust prompt strategy based on object complexity

CONCLUSION:
-----------
This experiment successfully compared {len(strategies)} prompt strategies on the iSAID
remote sensing dataset using Segment Anything Model (SAM). The results provide
quantitative evidence for prompt strategy selection in geospatial applications.
"""
...
print("1. Review the results in 'results/' folder")
print("2. Use the figure in 'figures/' for your paper")
print("3. Copy the statistical summary into your Results section")
print("4. Update your Methods section with experiment details")
print("5. Add references to reach ~40 total")
